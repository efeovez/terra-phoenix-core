...
func (app *TerraApp) RegisterUpgradeHandlers() {
	...
	app.Keepers.UpgradeKeeper.SetUpgradeHandler(
		...
		terraappconfig.Upgrade2_14,
		v2_14.CreateUpgradeHandler(
			app.GetModuleManager(),
			app.GetConfigurator(),
			app.Keepers,
		),
	...
	app.Keepers.UpgradeKeeper.SetUpgradeHandler(
		...
		terraappconfig.Upgrade2_15,
		v2_15.CreateUpgradeHandler(
			app.GetModuleManager(),
			app.GetConfigurator(),
		),
	)
	...
	app.Keepers.UpgradeKeeper.SetUpgradeHandler(
		terraappconfig.Upgrade2_18,
		v2_18.CreateUpgradeHandler(
			app.GetModuleManager(),
			app.GetConfigurator(),
			app.Keepers,
		),
...
func (app *TerraApp) RegisterUpgradeStores() {
	...
	if upgradeInfo.Name == terraappconfig.Upgrade2_3_0 && !app.Keepers.UpgradeKeeper.IsSkipHeight(upgradeInfo.Height) {
		...
		storeUpgrades := storetypes.StoreUpgrades{}
		app.SetStoreLoader(upgradetypes.UpgradeStoreLoader(upgradeInfo.Height, &storeUpgrades))
	} else if upgradeInfo.Name == terraappconfig.Upgrade2_20 && !app.Keepers.UpgradeKeeper.IsSkipHeight(upgradeInfo.Height) {
		storeUpgrades := storetypes.StoreUpgrades{Deleted: []string{"crisis"}}
		app.SetStoreLoader(upgradetypes.UpgradeStoreLoader(upgradeInfo.Height, &storeUpgrades))
	}
}
