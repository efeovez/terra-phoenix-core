...
CONTRACT_PATH=$(pwd)/integration-tests/src/contracts/counter.wasm
DENOM=uluna
SOFTWARE_UPGRADE_NAME="v2.20"
GOV_PERIOD="3s"
...
then
    ...
    mkdir -p build
    go build -mod=readonly -tags "netgo ledger" -trimpath -o build/terrad ./cmd/terrad
...
GOV_ADDRESS=$($OLD_BINARY query auth module-account gov --home $CHAIN_HOME --output json | jq .account.value.address -r)
echo '{
  "messages": [
    {
...
while true; do
    BLOCK_HEIGHT=$($OLD_BINARY status --home $CHAIN_HOME | jq '.sync_info.latest_block_height' -r)
    if [ $BLOCK_HEIGHT = "$UPGRADE_HEIGHT" ]; then
        ...
        STATUS=$($OLD_BINARY query gov proposal 1 --output=json --home $CHAIN_HOME | jq ".proposal.status" -r)
